...
**Najpierw zawęź obszar.** Jeśli user nie wskazał konkretnego modułu/warstwy — zapytaj: „Który obszar Cię boli / gdzie chcesz zacząć?" zanim spawn'ujesz Explore. Nieograniczony skan całego repo na trigger „popraw architekturę" pali tokeny i zwraca listę zbyt szeroką do decyzji.

Przeczytaj słownik domeny projektu i ADR-y w obszarze, którego dotykasz.

Potem użyj narzędzia Agent z `subagent_type=Explore` (scoped do wskazanego obszaru) żeby przeszedł przez codebase. Nie podążaj za sztywnymi heurystykami — eksploruj organicznie i zanotuj gdzie czujesz friction:
...
Przedstaw ponumerowaną listę **3-5 najsilniejszych** okazji do pogłębienia (jeśli znalazłeś więcej — wymień pozostałe jednolinijkowo jako „dalsze sygnały", bez pełnego szablonu). Dla każdego kandydata:
...
- **Nazwa pogłębionego modułu po koncepcji której nie ma w `CONTEXT.md`?** Dorzuć termin do `CONTEXT.md` — ta sama dyscyplina co `/grill` (zobacz [CONTEXT-FORMAT.md](../grill/CONTEXT-FORMAT.md)). Stwórz plik leniwie jeśli nie istnieje.
...
### 4. Wyjście — handoff do implementacji

Przegrillowany kandydat nie zostaje w powietrzu — routuj jawnie:

- **Duży refactor** (nowy moduł, cross-cutting, wiele slice'ów pracy) → `/to-prd` (kandydat + ustalenia z grillingu stają się inputem PRD).
- **Mały-średni refactor** (1-3 pliki, jeden slice) → `/code-manager` Tryb 4A (full plan) albo — przy pracy solo — bezpośrednia implementacja z planem w `doc/plans/<branch>.md`.
- **Kandydat odrzucony z load-bearing reason** → ADR (już masz to w Kroku 3).

Zamknij sesję podsumowaniem: wybrany kandydat, kształt docelowy, dokąd routujesz i co jest pierwszym krokiem.
